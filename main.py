...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    msg = (
        ...
        "한글:\n"
        "/시작 /도움말 /랭킹\n"
        "마피아: /마피아시작 /마피아목록 /참가 /마피아강제시작 /살해 /치료 /조사 /투표\n"
        "RPG: /rpg생성 /rpg직업선택 /rpg상태 /던전 /상점 /인벤토리 /스킬목록 /스킬습득 /내정보"
    )
...
async def mafia_list_join_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not data.startswith("mafia_join_"):
        await q.edit_message_text("잘못된 세션 참가 콜백.")
        return
    ...
    if not sess or sess["status"] != "waiting":
        await q.edit_message_text("해당 세션이 없거나 이미 시작.")
        c.close()
        conn.close()
        return
...
async def mafia_start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.effective_chat.type not in ("group", "supergroup"):
        await update.message.reply_text("그룹에서만 가능.")
        return
    ...
    """, (session_id, "waiting", group_id, 60, 30))
    conn.commit()
    c.close()
    conn.close()
...
async def mafia_force_start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(players) < 5:
        await update.message.reply_text("최소 5명 필요.")
        c.close()
        conn.close()
        return
    ...
    for pid in players:
        ...
        if ro == "Mafia":
            ...
            txt = "당신은 [시민]"
        try:
            await context.bot.send_message(pid, text=txt)
        except:
            pass
...
async def mafia_kill_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text("유효ID 아님.")
        return
...
async def mafia_doctor_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.effective_chat.type != "private":
        await update.message.reply_text("개인 채팅.")
        return
    args = context.args
    if len(args) < 2:
...
async def mafia_police_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    c.close()
    conn.close()
    await update.message.reply_text(f"{tgt}님을 조사 대상으로 설정.")
...
async def mafia_vote_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not rr or not rr["is_alive"]:
        await update.message.reply_text("당신은 죽었거나 참가X.")
        c.close()
        conn.close()
        return
    c.execute("""
    ...
    c.close()
    conn.close()
    await update.message.reply_text(f"{tgt}님에게 투표했습니다.")
...
async def rpg_create_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    c.close()
    conn.close()
    await update.message.reply_text("캐릭터 생성 완료! /rpg직업선택 으로 직업을 정하세요.")
...
async def rpg_job_callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not data.startswith("rpg_job_"):
        await q.edit_message_text("잘못된 직업콜백.")
        return
...
async def rpg_dungeon_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not data.startswith("rdsel_"):
        await q.edit_message_text("던전 콜백오류.")
        return
    ...
    kb = [
        [InlineKeyboardButton("👊 Attack", callback_data=f"rfd_{uid}_atk"),
    ...
    if not inv:
        ...
        for i in inv:
            txt += (
                ...
                f"(ATK+{i['atk_bonus']},HP+{i['hp_bonus']})\n"
            )
    await update.message.reply_text(txt)
...
async def rpg_skill_list_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for s in skills:
        text += (
            f"ID:{s['skill_id']} {s['name']} "
            f"(LvReq:{s['required_level']}, dmg:{s['damage']}, heal:{s['heal']})\n"
        )
...
async def rpg_skill_learn_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    srow = c.fetchone()
    if not srow:
        await update.message.reply_text("없는스킬 or 직업안맞음.")
        c.close()
        conn.close()
        return
