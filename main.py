...
########################################
def get_db_conn():
    """PostgreSQL 연결(매번 새 커넥션, 실제 운영에선 풀 사용 권장)."""
    conn = psycopg2.connect(DB_URL, cursor_factory=RealDictCursor)
    return conn

def init_db():
    """
    테이블 생성 + 필요한 칼럼 있는지 확인 후 없으면 추가(간단 마이그레이션).
    *AI 관련 부분은 전부 제거했습니다.*
    """
    conn = get_db_conn()
    cur = conn.cursor()
...
########################################
# 4. 기본 명령어 (/start, /help, /announce 등)
########################################
...
/참가 <세션ID>
/마피아강제시작 <세션ID>
(개인DM) /살해 <세션ID> <유저ID>    : 마피아
(개인DM) /치료 <세션ID> <유저ID>   : 의사
(개인DM) /조사 <세션ID> <유저ID>   : 경찰
(그룹)   /투표 <세션ID> <유저ID>   : 낮 투표
...
async def admin_secret_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    args= context.args
    if not args:
        await update.message.reply_text("비밀키를 입력. 예) /adminsecret MYKEY")
        return
...
async def announce_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not msg:
        await update.message.reply_text("공지할 내용을 입력.")
        return
...
async def mafia_join_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"참가 완료. 현재 {c}명 참가.")
...
async def mafia_force_start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not sess or sess["status"]!="waiting":
        await update.message.reply_text("세션없거나 이미시작.")
        cur.close()
        conn.close()
        return
    # 플레이어
    ...
    if len(players)<5:
        await update.message.reply_text("최소5명(마피아1,경찰1,의사1,시민2이상).")
        cur.close()
        conn.close()
        return
    ...
    await update.message.reply_text(f"마피아 게임 시작! 세션:{session_id}, 첫밤")
    # 개인 알림
    for pid in players:
        conn2=get_db_conn()
        c2=conn2.cursor()
        ...
        ro=r2["role"]
        if ro=="Mafia":
            role_text="[마피아] 밤에 /살해 <세션ID> <유저ID>"
        elif ro=="Police":
            role_text="[경찰] 밤에 /조사 <세션ID> <유저ID>"
        elif ro=="Doctor":
            role_text="[의사] 밤에 /치료 <세션ID> <유저ID>"
        else:
            role_text="[시민]"
        try:
            await context.bot.send_message(pid, text=f"당신은 {role_text}")
        except:
            pass
...
async def resolve_night_actions(session_id, group_id, context: ContextTypes.DEFAULT_TYPE):
    ...
    for pol_id, suspect_id in police_investigates.items():
        cur.execute("SELECT role,is_alive FROM mafia_players WHERE session_id=%s AND user_id=%s",(session_id,suspect_id))
        srow=cur.fetchone()
        if srow:
            role_info=srow["role"]
            try:
...
async def mafia_kill_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.effective_chat.type!="private":
        await update.message.reply_text("개인DM에서만.")
        return
    args=update.args
    if len(args)<2:
    ...
    try:
        ...
        await update.message.reply_text("유효한 ID아님.")
        return
    user_id=update.effective_user.id

    conn=get_db_conn()
    cur=conn.cursor()
    cur.execute("SELECT role,is_alive FROM mafia_players WHERE session_id=%s AND user_id=%s",(session_id,user_id))
    row=cur.fetchone()
    if not row or row["role"]!="Mafia" or not row["is_alive"]:
        await update.message.reply_text("마피아가 아니거나 사망.")
        cur.close()
        conn.close()
        return
    cur.execute("UPDATE mafia_players SET vote_target=%s WHERE session_id=%s AND user_id=%s",(target_id,session_id,user_id))
    conn.commit()
    cur.close()
    conn.close()
    await update.message.reply_text(f"{target_id}님을 살해 타겟으로 설정.")

async def mafia_doctor_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.effective_chat.type!="private":
        await update.message.reply_text("개인DM.")
        return
    args=update.args
    if len(args)<2:
    ...
    try:
        ...
        await update.message.reply_text("유효하지않음.")
        return
    user_id=update.effective_user.id
    conn=get_db_conn()
    cur=conn.cursor()
    ...
    if not row or row["role"]!="Doctor" or not row["is_alive"]:
        ...
        conn.close()
        return
    cur.execute("UPDATE mafia_players SET heal_target=%s WHERE session_id=%s AND user_id=%s",(tgt,session_id,user_id))
    conn.commit()
    cur.close()
    conn.close()
    await update.message.reply_text(f"{tgt}님 치료 대상으로 설정.")
...
async def mafia_police_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not row or row["role"]!="Police" or not row["is_alive"]:
        await update.message.reply_text("경찰이 아니거나 사망.")
        cur.close()
        conn.close()
        return
    cur.execute("UPDATE mafia_players SET investigate_target=%s WHERE session_id=%s AND user_id=%s",(tgt,session_id,user_id))
    conn.commit()
    cur.close()
    conn.close()
...
async def mafia_vote_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    srow=cur.fetchone()
    if not srow or srow["status"]!="day":
        await update.message.reply_text("현재 낮이 아님.")
        cur.close()
        conn.close()
        return
    ...
    if not r or not r["is_alive"]:
        await update.message.reply_text("당신은 죽었거나 참가 안함.")
        cur.close()
        conn.close()
        return
    cur.execute("""
    ...
    SET vote_target=%s
    WHERE session_id=%s AND user_id=%s
    """,(tgt,session_id,user_id))
    conn.commit()
    cur.close()
    conn.close()

    await update.message.reply_text(f"{tgt}님에게 투표됨.")
...
async def rpg_create_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    row=cur.fetchone()
    if row:
        await update.message.reply_text("이미 캐릭터 존재.")
        cur.close()
        conn.close()
        return
    cur.execute("""
    ...
    await update.message.reply_text("캐릭터 생성 완료! /rpg직업선택 으로 직업을 골라보세요.")
...
async def rpg_job_callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("rpg_job_"):
        ...
        row=cur.fetchone()
        if not row:
            await query.edit_message_text("먼저 /rpg생성 하세요.")
            cur.close()
            conn.close()
            return
        if row["job"]!="none":
...
async def rpg_status_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    row=cur.fetchone()
    if not row:
        await update.message.reply_text("캐릭터 없음. /rpg생성")
        cur.close()
        conn.close()
        return
    job=row["job"]
...
async def rpg_dungeon_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("rpg_dungeon_"):
        ...
        row=cur.fetchone()
        if not row:
            await query.edit_message_text("캐릭터없음.")
            cur.close()
            conn.close()
            return
        php=row["hp"]
        ...
        if php<=0:
            ...
            while pexp>=(plev*100):
                ...
                leveled_up+=1
            if leveled_up>0:
                msg+=f"{leveled_up}번 레벨업! 레벨:{plev}, 스킬포인트+{leveled_up}\n"
        ...
        WHERE user_id=%s
        """,(php,pexp,plev,pgold,psp,patk,pmax,user_id))
        conn.commit()
        cur.close()
        conn.close()
...
async def rpg_shop_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    conn.close()
    if not items:
        await update.message.reply_text("상점에 아이템 없음.")
        return
    text="[상점 목록]\n"
    kb=[]
    for it in items:
...
async def rpg_shop_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("rpg_shop_buy_"):
        ...
        try:
            ...
            await query.edit_message_text("아이템ID오류.")
            return
        conn=get_db_conn()
        cur=conn.cursor()
        ...
        irow=cur.fetchone()
        if not irow:
            await query.edit_message_text("아이템 없음.")
            cur.close()
            conn.close()
            return
        price=irow["price"]
        ...
        cur.close()
        conn.close()
        await query.edit_message_text(f"{irow['name']} 구매완료! (-{price} gold)")
...
async def rpg_skill_learn_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text("유효하지않은 스킬ID.")
        return
    user_id=update.effective_user.id
    conn=get_db_conn()
    cur=conn.cursor()
    ...
    srow=cur.fetchone()
    if not srow:
        await update.message.reply_text("직업 불일치거나 없는 스킬.")
        cur.close()
        conn.close()
        return
    if lv<srow["required_level"]:
        await update.message.reply_text("레벨 부족.")
        cur.close()
        conn.close()
        return
    if sp<1:
        await update.message.reply_text("스킬포인트 부족.")
        cur.close()
        conn.close()
        return
    # 이미 배움?
...
async def main():
    ...
    # 명령어 핸들러
    app.add_handler(CommandHandler(["start","시작"], start_command))
    app.add_handler(CommandHandler(["help","도움말"], help_command))
    ...
    app.add_handler(CommandHandler(["랭킹"], ranking_command))

    # 마피아
    app.add_handler(CommandHandler(["마피아시작"], mafia_start_command))
    app.add_handler(CommandHandler(["참가"], mafia_join_command))
    app.add_handler(CommandHandler(["마피아강제시작"], mafia_force_start_command))
    app.add_handler(CommandHandler(["살해"], mafia_kill_command))
    app.add_handler(CommandHandler(["치료"], mafia_doctor_command))
    app.add_handler(CommandHandler(["조사"], mafia_police_command))
    app.add_handler(CommandHandler(["투표"], mafia_vote_command))

    # RPG
    app.add_handler(CommandHandler(["rpg생성"], rpg_create_command))
    app.add_handler(CommandHandler(["rpg직업선택"], rpg_set_job_command))
    app.add_handler(CommandHandler(["rpg상태"], rpg_status_command))
    app.add_handler(CommandHandler(["던전"], rpg_dungeon_command))
    app.add_handler(CommandHandler(["상점"], rpg_shop_command))
    app.add_handler(CommandHandler(["인벤토리"], rpg_inventory_command))
    app.add_handler(CommandHandler(["스킬목록"], rpg_skill_list_command))
    app.add_handler(CommandHandler(["스킬습득"], rpg_skill_learn_command))
