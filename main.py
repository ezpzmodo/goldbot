...
async def rpg_set_job_command(update:Update, context:ContextTypes.DEFAULT_TYPE):
    kb=[
      [InlineKeyboardButton("전사",callback_data="rpgjob_warrior")],
      [InlineKeyboardButton("마법사",callback_data="rpgjob_mage")],
      [InlineKeyboardButton("도적",callback_data="rpgjob_thief")],
    ]
    await update.message.reply_text("직업을 선택:",reply_markup=InlineKeyboardMarkup(kb))

async def rpg_job_callback_handler(update:Update, context:ContextTypes.DEFAULT_TYPE):
    q=update.callback_query
    data=q.data
    await q.answer()
    if not data.startswith("rpgjob_"):return
    job=data.split("_")[1]
    uid=q.from_user.id
    conn=get_db_conn()
    c=conn.cursor()
    ...
    row=c.fetchone()
    if not row:
        await q.edit_message_text("먼저 /rpg생성")
        c.close();conn.close();return
    if row["job"]!="none":
    ...
    conn.commit()
    c.close();conn.close()
    await q.edit_message_text(f"{job} 직업 선택완료!")
...
async def rpg_shop_command(update:Update, context:ContextTypes.DEFAULT_TYPE):
    ...
    for it in items:
        ...
        kb.append([InlineKeyboardButton(f"{it['name']} 구매", callback_data=f"rpgshop_buy_{it['item_id']}")])
...
async def rpg_shop_callback(update:Update, context:ContextTypes.DEFAULT_TYPE):
    ...
    data=q.data
    await q.answer()
    if not data.startswith("rpgshop_buy_"):return
    iid=data.split("_")[2]
    try:
        item_id=int(iid)
    except:
...
async def rpg_skill_learn_command(update:Update, context:ContextTypes.DEFAULT_TYPE):
    ...
    srow=c.fetchone()
    if not srow:
        await update.message.reply_text("없는스킬 or직업불일치.")
        c.close();conn.close();return
    ...
    lr=c.fetchone()
    if lr:
        await update.message.reply_text("이미습득.")
        c.close();conn.close();return
    ...
    conn.commit()
    c.close();conn.close()
    await update.message.reply_text("스킬습득완료!")
...
async def rpg_fight_action_callback(update:Update, context:ContextTypes.DEFAULT_TYPE):
    ...
    if str(uid)!=owner_str:
        await q.answer("이 전투는 당신껀 아님!", show_alert=True)
        return
    st=rpg_fight_state.get(uid)
    if not st or st["phase"]!="ongoing":
        await q.answer("전투가 종료되었거나 없음.")
        return
    ...
    if action=="run":
        ...
        if action.startswith("useSkill"):
            ...
            try:
                ...
                await q.answer("스킬ID파싱에러",show_alert=True)
                return
            skill=None
            for s in skills:
                if s["skill_id"]==sid:
            ...
            if p_hp<=0:
                ...
                st["phase"]="end"
                await rpg_fight_victory(uid,monster,q,var_dmg,dmg_m,m_hp,p_hp,skillUsed=True)
                return
            else:
                kb=[[InlineKeyboardButton("👊 Attack",callback_data=f"rfd_{uid}_atk"),
                     InlineKeyboardButton("🔥 Skill",callback_data=f"rfd_{uid}_skill"),
                     InlineKeyboardButton("🏃 Run",callback_data=f"rfd_{uid}_run")]]
                txt=(f"{monster}HP:{m_hp},내HP:{p_hp}\n"
                     f"(스킬사용 dmg:{var_dmg}, heal:{var_heal},몬공격:{dmg_m})\n"
                     "행동선택:")
                await q.edit_message_text(txt, reply_markup=InlineKeyboardMarkup(kb))
        else:
...
async def rpg_fight_victory(uid:int, monster:str, query, dmg_p:int, dmg_m:int, m_hp:int, p_hp:int, skillUsed=False):
    ...
    txt=(f"{monster} 처치!\n"
         f"획득: EXP+{reward_exp}, GOLD+{reward_gold}{lvup_txt}\n"
         "전투종료!")
